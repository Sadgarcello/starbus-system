...
def rankings():
    ...
    cur = conn.cursor()
    try:
        # Pull image_profile from fighters and join to the leaderboard
        rows = cur.execute("""
            SELECT
                l.id,
                l.name,
...
def rankings():
    ...
    try:
        ...
        last_updated = ""
        if last_raw:
            # Try to present as "YYYY-MM-DD HH:MM"
            try:
...
if __name__ == "__main__":
    app.run(debug=True)
return render_template("rankings.html", fighters=rows)
# --------------------------------------------------------------------
